...
/// Long timeout for the commit endpoint (docs: up to 30 minutes).
...
impl RoleLogicClient {
    ...
    /// Atomic full replace for lists of up to 100 000 users.
    ...
    /// Full replace that transparently switches to the chunked-upload flow
    /// when the list exceeds the 100 000 per-PUT limit. The chunked flow is
    /// atomic — the live list is only swapped in on commit.
